...
public enum TodoDAO{
            ...
            private final String INSERT="INSERT INTO tb1_todo(title,memo,dueDate) VALUES(?, ?, ?)";
            private final String DELETE="DELETE FROM tb1_todo WHERE tno=?";

            private final String LAST = "select last_insert_id()";

            public List<TodoDTO> list(int page, int size)throws Exception{
                StringBuffer buffer =
                new StringBuffer("select * from webdb.tbl_todo where tno >0  order by tno desc limit ?,?");
